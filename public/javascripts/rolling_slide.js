...
setInterval(() => {
    rolling_list_box.classList.add(TRANS_TOP);
    rolling_list_box.classList.add(ON);
    setTimeout (() => {
        rolling_list_box.insertBefore(rolling_list_box.firstElementChild, null);
        rolling_list_box.classList.remove(TRANS_TOP);
        rolling_list_box.classList.remove(ON);
    },300)
},3000);